...
pub fn parse_interactables(path: String, renderer_data: &RendererData) -> Result<Vec<Interactable>> {
    read_interactables_from_file(path, renderer_data)
}

pub fn read_interactables_from_file(path: String, renderer_data: &RendererData) -> Result<Vec<Interactable>> {
        ...
        match reader.read_event_into(&mut buf)? {
            Event::Start(ref e) if e.name().as_ref() == b"element" => {
                ...
                match (element_type.as_deref(), label.as_str()) {
                    (Some("point"), label) if label.starts_with("Interactable") => {
                        read_values_for_enemy_from_point(&mut reader, &mut buf, &mut Interactable
                            , renderer_data)?
                    }
                    _ => {}
                }
