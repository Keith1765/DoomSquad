...
pub fn generate_entities (entity_type: EntityType, position: Point, height: f64, facing_direction: f64, renderer_data: &RendererData) -> Entity {
    match entity_type {
        ...
        EntityType::Dummy => Entity::new(position, height, DUMMY_SIZE, facing_direction, 0, renderer_data, EntityType::Dummy, DUMMY_HP, DUMMY_SIZE).unwrap(),
        EntityType::RangedEnemy => Entity::new(position, height, ENEMY_SIZE, facing_direction, 3, renderer_data, EntityType::RangedEnemy, ENEMY_HP, ENEMY_SIZE).unwrap(),
        EntityType::Archer => Entity::new(position, height, ENEMY_SIZE, facing_direction, 3, renderer_data, EntityType::Archer, ENEMY_HP, ENEMY_SIZE).unwrap(),
        EntityType::MeleeEnemy => Entity::new(position, height, ENEMY_SIZE, facing_direction, 4, renderer_data, EntityType::MeleeEnemy, ENEMY_HP, ENEMY_SIZE).unwrap(),
