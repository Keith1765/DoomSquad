...
pub fn draw_crosshair (buffer: &mut [u32], vertical_aim_offset: f64, renderer_data: &RendererData, aim_mode: bool) {
    ...
    for x in -CROSSHAIR_SIZE/2..=CROSSHAIR_SIZE/2{
        for y in 0..CROSSHAIR_THICKNESS {
            buffer[((middle_point.y + crosshair_vertical_offset)as usize + y)*SCREEN_WIDTH + (middle_point.x as isize + x) as usize] = crosshair_color;
        }
    }
    ...
    for y in -CROSSHAIR_SIZE/2..=CROSSHAIR_SIZE/2{
        for x in 0..CROSSHAIR_THICKNESS {
            buffer[((middle_point.y + crosshair_vertical_offset)as isize + y) as usize *SCREEN_WIDTH + (middle_point.x as usize + x)] = crosshair_color;
        }
    }
